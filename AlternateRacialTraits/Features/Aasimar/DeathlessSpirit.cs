            ...
            return
                InitContext.NewBlueprint<BlueprintFeature>(guid, nameof(DeathlessSpirit))
                    .Map(blueprint =>
                    {
                        ...
                        blueprint.AddAddDamageResistanceEnergy(c =>
                        {
                            c.Type = DamageEnergyType.NegativeEnergy;
                            c.Value = 5;
                        });

                        blueprint.AddSavingThrowBonusAgainstDescriptor(c =>
                        {
                            c.Value = 2;
                        ...
                        blueprint.AddComponent<DeathlessSpiritComponent>();

                        blueprint.AddPrerequisiteFeature(BlueprintsDb.Owlcat.BlueprintFeature.CelestialResistance, removeOnApply: true);
