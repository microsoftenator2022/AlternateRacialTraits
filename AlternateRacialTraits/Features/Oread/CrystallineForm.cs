...
internal static partial class CrystallineForm
{
        ...
        IUnitEquipmentHandler, IUnitActiveEquipmentSetHandler, IUnitAbilityResourceHandler, IUnitDeflectedRayHandler
    {
        ...
        public override void OnTurnOn() => this.UpdateState();
        ...
        class Patches
        {
            ...
            static bool IsFromWeaponOrCanDeflectRay(Projectile projectile) =>
                CanDeflectRayWeapon(projectile) || projectile.IsFromWeapon;
            ...
            {
                if (__result)
                    _ = EventBus.RaiseEvent<IUnitDeflectedRayHandler>(handler => handler.HandleUnitDeflectedRay(projectile.Target.Unit, projectile));
            }
    ...
    internal static IDeferred<BlueprintFeature> Create()
    {
        ...
        var feature = Deferred.NewBlueprint<BlueprintFeature>(GeneratedGuid.Get("CrystallineForm"))
            ...
            .Map(bps =>
            {
                ...
                _ = feature.AddACBonusAgainstWeaponCategory(c =>
                {
                    c.ArmorClassBonus = 2;
                ...
                _ = feature.AddComponent<DeflectRay>(deflect =>
                {
                    deflect.RayWeapon = ray.ToReference();
                    deflect.Resource = resource.ToReference();
                });

                _ = feature.AddAddAbilityResources(addResources =>
                {
                    addResources.Amount = 1;
                    addResources.m_Resource = resource.ToReference();
                });

                _ = feature.AddPrerequisiteFeature(BlueprintsDb.Owlcat.BlueprintFeature.AcidAffinityOread, true);
